...
exports.vehicleEntry = async (req, res) => {
    try {
        ...
        if (!vehicle || (qrCode && vehicle.qrCode !== qrCode)) {
            await FraudLog.create({ numberPlate: plateNumber, qrCode: qrCode || null, reason: "Plate/QR mismatch or unregistered vehicle" });
        ...
        await Notification.create({ userId: vehicle.userId, title: "Vehicle Entry", message: `${plateNumber} entered parking at ${new Date().toLocaleTimeString()}`, type: "ENTRY" });
    ...
    try {
        if (req.user.role === "Admin") {
            const logs = await EntryExitLog.find()
                .populate({ path: "vehicleId", select: "plateNumber userId", populate: { path: "userId", select: "name phone email" } })
                .sort({ createdAt: -1 });
...
exports.getMyParkingStatus = async (req, res) => {
    try {
        ...
        const vehicleIds = myVehicles.map(v => v._id);
        const activeLog = await EntryExitLog.findOne({ vehicleId: { $in: vehicleIds }, status: "IN" }).populate("vehicleId", "plateNumber");
        if (!activeLog) return res.json({ parked: false, data: null });
        const minutes = Math.ceil((Date.now() - activeLog.entryTime) / 60000);
        const hours = Math.ceil(minutes / 60);
        res.json({ parked: true, data: { plateNumber: activeLog.vehicleId?.plateNumber, entryTime: activeLog.entryTime, duration: `${Math.floor(minutes / 60)}h ${minutes % 60}m`, estimatedAmount: hours * RATE_PER_HOUR } });
    } catch (err) {
        res.status(500).json({ message: err.message });
    }
};